...
st.markdown("**Empire core** • Launch/edit accounts with unified trees • Contributor Pool enforced • Exact 100% validation • Auto v2 migration • Realtime previews • Bulletproof sync • Optional Growth Fund %")
...
def fetch_ftmo_data():
    ...
    special = ["Contributor Pool", "Manual Payout (Temporary)", "Growth Fund"]
    for s in special:
...
if current_role in ["owner", "admin"]:
    ...
    with st.expander("➕ Launch New FTMO Account", expanded=True):
        with st.form("create_ftmo", clear_on_submit=True):
            ...
            gf_pct = st.number_input("Growth Fund % from Gross Profit", 0.0, 50.0, 10.0, 0.5)
            if gf_pct > 0:
            ...
            st.info("Must include **exactly one** 'Contributor Pool' row • Total + GF must be **exactly 100%**")
            ...
            if submitted:
                if not name.strip():
                    ...
                    st.error(f"Total % including Growth Fund must be exactly 100.00% (current: {total_with_gf:.2f}%)")
                else:
                    try:
                        final_part_v2 = []
